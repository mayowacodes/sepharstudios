...
import { d as db, m as mediaLibrary, C as quizSessions } from './drizzle-CW7hPjGG.js';
...
const POST = async ({ request, locals }) => {
  ...
  if (!private_env.OLLAMA_URL) {
    const [session_2] = await db.insert(quizSessions).values({
      profileId: resolvedProfileId,
      contentId,
      questions: defaultQuestions
    }).returning();
    return json({ sessionId: session_2.id, questions: defaultQuestions.map((q) => ({ question: q.question, options: q.options })) });
  }
  try {
    ...
    if (ollamaRes.ok) {
      ...
      if (match) {
        ...
        if (Array.isArray(questions) && questions.length > 0) {
          const [session_2] = await db.insert(quizSessions).values({ profileId: resolvedProfileId, contentId, questions }).returning();
          return json({ sessionId: session_2.id, questions: questions.map((q) => ({ question: q.question, options: q.options })) });
        }
      }
    }
  } catch {
...
export { POST };
//# sourceMappingURL=_server.ts-jGLJBdTL.js.map
