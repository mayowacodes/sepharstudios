import { d as db, aw as account } from './drizzle-C3SH12nS.js';
...
//#region src/routes/api/users/me/auth-providers/+server.ts
/**
...
var GET = async ({ locals }) => {
	...
	const [hasCred] = await db.select({ id: account.id }).from(account).where(and(eq(account.userId, session.user.id), eq(account.providerId, "credential"), isNotNull(account.password))).limit(1);
	return json({
		providers,
		hasPassword: !!hasCred
	});
...
export { GET };
//# sourceMappingURL=_server.ts-CwoRsapb.js.map
