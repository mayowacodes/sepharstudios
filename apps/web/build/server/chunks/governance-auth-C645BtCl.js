import { d as db, j as governanceProposals, k as governanceProposalApprovals, l as governanceAuditEntries, o as governancePauseEvents, c as user, q as governanceMemberships } from './drizzle-CW7hPjGG.js';
import { eq, and, count, desc, inArray, asc, isNotNull, lte } from 'drizzle-orm';

async function getAdminActor(locals) {
  const session = await locals.auth.getSession();
  if (!session) return null;
  const [account] = await db.select({ id: user.id, role: user.role, name: user.name, email: user.email }).from(user).where(eq(user.id, session.user.id)).limit(1);
...
const TIMESTAMP_MS_ROUTINE = 72 * 60 * 60 * 1e3;
const TIMESTAMP_MS_MONETARY = 7 * 24 * 60 * 60 * 1e3;
...
const DEFAULT_GOVERNANCE_PERMISSIONS = [
  "governance.view",
...
async function ensureQueueMaturity() {
  await db.update(governanceProposals).set({
    ...
    and(
      ...
      lte(governanceProposals.eta, /* @__PURE__ */ new Date())
    )
  );
}
...
async function listAuditEntries() {
  const rows = await db.select().from(governanceAuditEntries).orderBy(desc(governanceAuditEntries.createdAt));
  return rows.map((row) => ({
    id: row.id,
...
async function approveProposal(id, actor) {
  ...
  if (proposalRow.status !== "submitted") {
    const [proposal2] = await hydrateProposals([proposalRow]);
    return proposal2;
  }
  const [existing] = await db.select({ id: governanceProposalApprovals.id }).from(governanceProposalApprovals).where(
    and(
...
async function listQueue() {
  await ensureQueueMaturity();
  const rows = await db.select().from(governanceProposals).where(inArray(governanceProposals.status, ["queued", "executable"])).orderBy(asc(governanceProposals.eta), asc(governanceProposals.createdAt));
  return hydrateProposals(rows);
}
...
async function listPauseEvents() {
  const rows = await db.select().from(governancePauseEvents).orderBy(desc(governancePauseEvents.triggeredAt));
  return rows.map((row) => ({
    id: row.id,
    reason: row.reason,
    triggeredBy: row.triggeredBy,
...
async function getGovernancePermissionsForUser(userId) {
  const [membership] = await db.select({ permissions: governanceMemberships.permissions, active: governanceMemberships.active }).from(governanceMemberships).where(eq(governanceMemberships.userId, userId)).limit(1);
...
export { DEFAULT_GOVERNANCE_PERMISSIONS as D, approveProposal as a, listPauseEvents as b, getActivePause as c, listProposals as d, executeProposal as e, createProposal as f, getGovernanceActor as g, listQueue as h, listAuditEntries as l, queueProposal as q, triggerPause as t };
//# sourceMappingURL=governance-auth-C645BtCl.js.map
