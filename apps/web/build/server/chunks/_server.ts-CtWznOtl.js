import { d as db, b as abuseReports, r as reviews, f as forumThreads, e as forumReplies, m as mediaLibrary, c as user } from './drizzle-DlGuU73K.js';
...
//#region src/routes/api/reports/+server.ts
/**
...
var POST = async ({ request, locals, getClientAddress }) => {
	...
	if (!(await take(`reports:${session?.user.id ?? `ip:${getClientAddress()}`}`, {
		capacity: 10,
		refillPerSec: 1 / 360
	})).allowed) return json({ error: "Too many reports, try again later." }, { status: 429 });
	const body = await request.json().catch(() => ({}));
	const targetType = body.targetType ?? "";
	const targetId = body.targetId?.trim() ?? "";
	const category = body.category ?? "";
	if (!ALLOWED_TARGET_TYPES.has(targetType)) return json({ error: "Invalid target type" }, { status: 400 });
	if (!targetId) return json({ error: "targetId is required" }, { status: 400 });
	if (!ALLOWED_CATEGORIES.has(category)) return json({ error: "Invalid category" }, { status: 400 });
	if (!await targetExists(targetType, targetId)) return json({ error: "Target not found" }, { status: 404 });
	const description = (body.description ?? "").trim().slice(0, 2e3);
...
export { POST };
//# sourceMappingURL=_server.ts-CtWznOtl.js.map
