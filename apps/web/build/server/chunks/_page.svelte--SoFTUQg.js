import { k as head, b as ensure_array_like, q as escape_html, x as attr_class, v as stringify, c as clsx$1 } from './index.js-CxPEndTa.js';
						...
						children: ($$renderer) => {
							if (data.byGeo.length === 0) {
								...
								for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
									...
									$$renderer.push(`<tr class="border-t"><td class="py-2">${escape_html(row.country ?? "—")}</td><td>${escape_html(row.deviceType ?? "—")}</td><td>${escape_html(row.sessions.toLocaleString())}</td><td>${escape_html(row.avgBitrateKbps)} kbps</td><td${attr_class(clsx$1(severity(row.sessions ? row.erroredSessions / row.sessions : 0, .05, .15)))}>${escape_html(pct(row.sessions ? row.erroredSessions / row.sessions : 0))}</td></tr>`);
								}
...
//# sourceMappingURL=_page.svelte--SoFTUQg.js.map
