...
import { d as db, h as creatorApplications, c as user, i as creators } from './drizzle-CW7hPjGG.js';
...
const PATCH = async ({ locals, params, request }) => {
  ...
  if (payload.status === "approved") {
    const [appUser] = await db.select().from(user).where(eq(user.id, application.userId));
    if (appUser) {
      ...
      const [existingProfile] = await db.select({ id: creators.id }).from(creators).where(eq(creators.userId, application.userId));
...
export { PATCH };
//# sourceMappingURL=_server.ts-CrhHhsUB.js.map
