...
const POST = async ({ params, request, locals }) => {
  ...
  if (action === "set") {
    ...
    await db.update(profiles).set({ pin: hashPin(pin), pinSetAt: /* @__PURE__ */ new Date(), updatedAt: /* @__PURE__ */ new Date() }).where(eq(profiles.id, params.id));
  ...
  if (action === "verify") {
    ...
    if (!valid && !profile.pin.startsWith("v1:")) {
      ...
      if (valid) {
        await db.update(profiles).set({ pin: hashPin(pin), updatedAt: /* @__PURE__ */ new Date() }).where(eq(profiles.id, params.id));
      }
    }
    return json({ valid });
  }
  if (action === "remove") {
    await db.update(profiles).set({ pin: null, pinSetAt: null, updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(profiles.id, params.id), eq(profiles.userId, session.user.id)));
    return json({ success: true });
  }
...
export { POST };
//# sourceMappingURL=_server.ts-D7dCSig8.js.map
