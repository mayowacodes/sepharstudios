import { ah as attr, au as escape_html, as as ensure_array_like, ai as attr_class, an as clsx$1, aj as attr_style, aR as stringify, ap as derived } from './ui-libs-BjzLDLAh.js';
...
function StepIndicator($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		let { steps, currentStep, isStepValid} = $$props;
		...
		for (let index = 0, $$length = each_array.length; index < $$length; index++) {
			...
			$$renderer.push(`<div${attr_class(`flex items-center ${index === steps.length - 1 ? "" : "flex-1"}`)}><div class="flex flex-col items-center"><button type="button"${attr_class(clsx$1(classesFor(step.step)))}${attr("disabled", step.step > currentStep && !isStepValid[step.step - 1], true)}${attr("aria-label", `Step ${step.step}: ${step.title} - ${statusFor(step.step)}`)}${attr("aria-current", step.step === currentStep ? "step" : void 0)}>`);
			...
			if (index < steps.length - 1) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<div${attr_class(clsx$1(connectorClasses(step.step)))}></div>`);
...
//# sourceMappingURL=_page.svelte-DlFWgTdP.js.map
