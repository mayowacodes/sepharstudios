import { p as private_env, j as json } from './index.js-CxPEndTa.js';
...
//#region src/routes/api/admin/ai/models/+server.ts
...
function getTags(modelId, price, contextLength) {
	...
	else if (price < .5) tags.push("cheap");
...
var GET = async ({ locals }) => {
	...
	if (modelsCache && Date.now() < modelsCacheExpiry) return json({
		models: modelsCache,
		hasApiKey
	});
	if (!hasApiKey) return json({
		models: buildStaticModelList(),
		hasApiKey: false
	});
	try {
		...
		if (!res.ok) return json({
			models: buildStaticModelList(),
			hasApiKey,
			warning: "Could not fetch live model list from OpenRouter"
		});
		const models = ((await res.json()).data ?? []).map((m) => {
		...
		modelsCache = models;
		modelsCacheExpiry = Date.now() + 3600 * 1e3;
		return json({
			models,
			hasApiKey
		});
		...
		return json({
			models: buildStaticModelList(),
			hasApiKey,
			warning: "OpenRouter request timed out — showing curated list"
		});
	}
};
	...
	return [
		{
			...
			description: "Google's Gemma 3 27B — free tier. Great for chat and faith-based conversations.",
			contextLength: 96e3,
			promptPrice: "Free",
		...
		},
		{
			...
			description: "DeepSeek's powerful reasoning model — free tier. Excellent for structured tasks.",
		...
		},
		{
			...
			description: "DeepSeek Chat — free tier. Fast and capable.",
			contextLength: 65536,
			promptPrice: "Free",
			completionPrice: "Free",
			isFree: true,
		...
		},
		{
			...
			description: "Meta's Llama 3.1 8B — free. Lightweight and fast.",
		...
		},
		{
			...
			description: "Mistral 7B Instruct — free. Reliable for structured tasks.",
			contextLength: 32768,
			promptPrice: "Free",
			completionPrice: "Free",
			isFree: true,
		...
		},
		{
			...
			description: "Google's latest fast model. Excellent quality for the price. Recommended for chat.",
		...
		},
		{
			...
			description: "Newer, smarter Gemini Flash — slightly higher cost but significantly better.",
			contextLength: 1048576,
			promptPrice: "$0.15",
		...
		},
		{
			...
			name: "DeepSeek R1 ⭐",
			description: "Near-GPT-4 quality reasoning. Recommended for agent/structured tasks at low cost.",
			contextLength: 163840,
			promptPrice: "$0.55",
		...
		},
		{
			...
			description: "Previous Gemini Flash — very fast and cheap.",
			contextLength: 1e6,
			promptPrice: "$0.08",
		...
		},
		{
			...
			name: "Llama 3.3 70B",
			description: "Meta's best open-source model. Strong for both chat and structured tasks.",
			contextLength: 131072,
			promptPrice: "$0.30",
			completionPrice: "$0.39",
			isFree: false,
			category: "both",
			tags: ["cheap", "large-context"]
		},
		{
			id: "openai/gpt-4o-mini",
			name: "GPT-4o Mini",
			description: "OpenAI's affordable fast model. Good balance of quality and cost.",
			contextLength: 128e3,
			promptPrice: "$0.15",
			completionPrice: "$0.60",
			isFree: false,
		...
		},
		{
			...
			description: "Anthropic's best model for nuanced conversations. Exceptional for theological discussions.",
			contextLength: 2e5,
			promptPrice: "$3.00",
		...
		},
		{
			id: "openai/gpt-4o",
			name: "GPT-4o",
			description: "OpenAI's flagship model. Best overall quality.",
			contextLength: 128e3,
			promptPrice: "$2.50",
		...
		},
		{
			...
			description: "Google's top model. Excellent for complex reasoning and long context.",
			contextLength: 1048576,
			promptPrice: "$1.25",
			completionPrice: "$10.00",
			isFree: false,
...
export { GET };
//# sourceMappingURL=_server.ts-DoDkW-XO.js.map
