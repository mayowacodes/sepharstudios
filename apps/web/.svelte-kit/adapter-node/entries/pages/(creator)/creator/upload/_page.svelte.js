import { At as clsx, St as stringify, dt as attr_style, gt as ensure_array_like, jt as escape_html, kt as attr, pt as bind_props, ut as attr_class } from "../../../../../chunks/ui-libs.js";
...
function StepIndicator($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		for (let index = 0, $$length = each_array.length; index < $$length; index++) {
			...
			$$renderer.push(`<div${attr_class(`flex items-center ${index === steps.length - 1 ? "" : "flex-1"}`)}><div class="flex flex-col items-center"><button${attr_class(clsx(getStepClasses(step.step)))}${attr("disabled", step.step > currentStep && !isStepValid[step.step - 1], true)}${attr("aria-label", `Step ${step.step}: ${step.title} - ${getStepStatus(step.step)}`)}${attr("aria-current", step.step === currentStep ? "step" : void 0)}>`);
			...
			if (index < steps.length - 1) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<div${attr_class(clsx(getConnectorClasses(step.step)))}></div>`);
