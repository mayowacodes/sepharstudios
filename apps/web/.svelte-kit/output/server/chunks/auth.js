...
//#endregion
//#region src/lib/db/account.ts
var getAccountByUserId = async (userId) => {
	try {
		return onSuccess(await db.select().from(account).where(eq(account.userId, userId)).limit(1).then((res) => res[0]));
	} catch (error) {
...
var auth = betterAuth({
	...
	plugins: [
		...
		customSession(async ({ user, session }) => {
			return {
				account: (await getAccountByUserId(user.id)).data,
				user,
				session
			};
		}),
