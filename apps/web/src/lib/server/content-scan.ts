...
export interface ScanAiVerdict {
 ...
 */
export async function runContentScan(mediaId: string): Promise<void> {
	...
	// Pull the transcript. Best-effort — proceed without if missing.
	// Prefer the multi-track shape (subtitles[]) with `default: true`; fall
	// back to the legacy single-transcript shape; finally any track.
	let transcript = '';
	let txtUrl: string | undefined;
	const subs = row.contentScanReport.subtitles;
	if (Array.isArray(subs) && subs.length > 0) {
		const defaultTrack = subs.find((s) => s.default) ?? subs[0];
		txtUrl = defaultTrack?.txtUrl;
	}
	if (!txtUrl) {
		txtUrl = row.contentScanReport.transcript?.txtUrl;
	}
	if (txtUrl) {
		try {
			const res = await fetch(txtUrl, { signal: AbortSignal.timeout(20_000) });
...
Pull up to 5 short transcriptExcerpts that the admin should specifically read.`
	...
	const verdict = tryParseJson<ScanAiVerdict>(result.content);
	if (!verdict) {
		await db.update(mediaLibrary)
			.set({
				contentScanStatus: 'failed',
				contentScanReport: { ...baseReport, completedAt: now } as typeof baseReport,
				updatedAt: new Date()
			})
			.where(eq(mediaLibrary.id, mediaId));
	...
	// Notify all admins on flag/reject. On approve we stay silent — admin
	// can still discover via the dashboard but no urgent ping needed.
	if (verdict.verdict === 'flag' || verdict.verdict === 'reject') {
		const title = verdict.verdict === 'reject'
			? `Scan rejected: "${row.title.slice(0, 50)}"`
			: `Scan flagged: "${row.title.slice(0, 50)}"`;
		await notifyAdmins({
			kind: 'system',
			title,
