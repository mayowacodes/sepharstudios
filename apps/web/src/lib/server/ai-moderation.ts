import { callAgent, extractJsonObject, SEPHAR_SYSTEM_PROMPT } from './ai-provider';

/**
 * AI MODERATION & SAFETY ENGINE  [Hermes 3 — structured JSON verdicts]
 */

export type ModerationVerdict = 'approve' | 'flag' | 'reject';

export interface CommentModerationResult {
	verdict: ModerationVerdict;
	reason: string;
	isSpam: boolean;
	isHarmful: boolean;
	isOffTopic: boolean;
	qualityScore: number;
	aiProvider: string;
}

export interface ContentModerationResult {
	verdict: ModerationVerdict;
	flags: string[];
	theologyScore: number;
	familySafeScore: number;
	reason: string;
	recommendedAgeRating: string;
	aiProvider: string;
}

export interface ReviewQualityResult {
	qualityScore: number;
	isGenuine: boolean;
	isThoughtful: boolean;
	hasFaithInsight: boolean;
	tokenRewardMultiplier: number;
	aiProvider: string;
}

export async function moderateComment(
	commentText: string,
	contentTitle: string
): Promise<CommentModerationResult | null> {
	const result = await callAgent(
		[
			{ role: 'system', content: SEPHAR_SYSTEM_PROMPT },
			{
				role: 'user',
				content: `Moderate this comment on a Christian streaming platform. The content title is: "${contentTitle}"

Comment: "${commentText}"

Return ONLY this JSON:
{
  "verdict": "approve",
  "reason": "Genuine thoughtful comment",
  "isSpam": false,
  "isHarmful": false,
  "isOffTopic": false,
  "qualityScore": 7
}

Verdict rules:
- "approve": genuine, relevant, family-safe
- "flag": borderline, needs human review
- "reject": spam, harmful, deeply off-topic, or abusive

qualityScore 0–10:
- 0–2: spam/gibberish/single word
- 3–5: generic ("good movie", "loved it")
- 6–8: thoughtful, specific to the content
- 9–10: detailed insight with faith reflection`
			}
		],
		{ temperature: 0.1, maxTokens: 256 }
	);

	if (!result) return null;
	const parsed = extractJsonObject<Omit<CommentModerationResult, 'aiProvider'>>(result.content);
	if (!parsed) return null;
	return { ...parsed, aiProvider: `${result.provider}/${result.model}` };
}

export async function moderateContentMetadata(
	title: string,
	description: string,
	genres: string[],
	topics: string[],
	bibleReference: string,
	contentType: string
): Promise<ContentModerationResult | null> {
	const result = await callAgent(
		[
			{ role: 'system', content: SEPHAR_SYSTEM_PROMPT },
			{
				role: 'user',
				content: `Pre-screen this content submission for a faith-based Christian streaming platform.

Title: "${title}"
Type: ${contentType}
Description: "${description}"
Genres: ${genres.join(', ')}
Topics: ${topics.join(', ')}
Bible Reference: "${bibleReference}"

Return ONLY this JSON:
{
  "verdict": "approve",
  "flags": [],
  "theologyScore": 8,
  "familySafeScore": 9,
  "reason": "Aligns with Christian values, family appropriate",
  "recommendedAgeRating": "All"
}

Scoring:
- theologyScore 0–10: does content align with core Christian doctrine?
- familySafeScore 0–10: 10 = all ages, 0 = extremely inappropriate
- verdict: "approve" | "flag" | "reject"
- flags: concerns for human reviewers`
			}
		],
		{ temperature: 0.2, maxTokens: 512 }
	);

	if (!result) return null;
	const parsed = extractJsonObject<Omit<ContentModerationResult, 'aiProvider'>>(result.content);
	if (!parsed) return null;
	return { ...parsed, aiProvider: `${result.provider}/${result.model}` };
}

export async function scoreReviewQuality(
	reviewText: string,
	rating: number,
	contentTitle: string
): Promise<ReviewQualityResult | null> {
	const result = await callAgent(
		[
			{ role: 'system', content: SEPHAR_SYSTEM_PROMPT },
			{
				role: 'user',
				content: `Score this user review on a Christian streaming platform for quality and authenticity.
This score determines how many STC tokens the reviewer earns.

Content: "${contentTitle}"
Star Rating: ${rating}/5
Review: "${reviewText}"

Return ONLY this JSON:
{
  "qualityScore": 7,
  "isGenuine": true,
  "isThoughtful": true,
  "hasFaithInsight": false,
  "tokenRewardMultiplier": 1.0
}

Scoring rules:
- qualityScore 0–10 based on length, specificity, and engagement
- isGenuine: false if bot, copy-paste, or reward farming
- isThoughtful: true if it references actual content details
- hasFaithInsight: true if it mentions biblical themes or faith lessons
- tokenRewardMultiplier: 0.0 = spam, 0.5 = generic, 1.0 = solid, 1.5 = faith-engaged`
			}
		],
		{ temperature: 0.1, maxTokens: 256 }
	);

	if (!result) return null;
	const parsed = extractJsonObject<Omit<ReviewQualityResult, 'aiProvider'>>(result.content);
	if (!parsed) return null;
	return { ...parsed, aiProvider: `${result.provider}/${result.model}` };
}
