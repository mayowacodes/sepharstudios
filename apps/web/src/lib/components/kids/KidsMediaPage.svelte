<script lang="ts">
  ...
  let { title, mediaData }: { title: string; mediaData: any[] } = $props();

  // Compute derived views from `mediaData` reactively so re-renders with a
  // new dataset don't keep showing the first one.
