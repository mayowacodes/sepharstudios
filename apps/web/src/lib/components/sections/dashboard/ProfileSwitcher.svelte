<script lang="ts">
  ...
  let editProfile = $state<Profile | null>(null);
...
<Sheet bind:open>
  ...
  <SheetContent side="bottom" class="p-6 sm:max-w-md mx-auto">
    <h2 class="text-lg font-semibold mb-4 text-center">Manage Profiles</h2>
      ...
      {#each $profileSlots as profile (profile.id)}
        <div class="flex flex-col items-center gap-2 p-3 bg-muted rounded-xl hover:bg-muted/70 transition cursor-pointer relative">
          <div class="relative group">
            <button
              class="h-14 w-14 rounded-full bg-primary flex items-center justify-center text-white text-lg font-bold"
              onclick={() => switchProfile(profile)}
            >
              {profile.name[0]}
            </button>
            <button
              class="absolute -bottom-1 -right-1 bg-background rounded-full p-1 shadow-sm cursor-pointer group-hover:scale-110 border"
          ...
          <span class="text-[10px] text-muted-foreground capitalize">
            {profile.type} {profile.parental ? '(KIDS)' : ''}
          </span>
        </div>
      {/each}
    </div>
    ...
    {#if $profileSlots.length < maxProfiles}
      <button
        class="flex items-center justify-center w-full gap-2 text-sm font-medium text-primary hover:underline mt-4"
        onclick={() => (showAddModal = true)}
      >
        <PlusCircle class="w-4 h-4" /> Add New Profile
      </button>
    {:else}
      <p class="text-xs text-muted-foreground text-center">Maximum of 8 profiles reached.</p>
    {/if}
  </SheetContent>
</Sheet>
