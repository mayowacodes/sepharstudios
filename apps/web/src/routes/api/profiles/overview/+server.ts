 ...
 */
async function buildProfilesPayload({ locals }: RequestEvent) {
...
export type ProfilesOverviewPayload = Awaited<ReturnType<typeof buildProfilesPayload>>;

export const GET: RequestHandler = async (event) => json(await buildProfilesPayload(event));
