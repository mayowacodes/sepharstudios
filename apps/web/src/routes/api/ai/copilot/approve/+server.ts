...
import { aiActionLog, copilotMessages } from '$lib/db/schema/sepharstudios';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const [row] = await db.select()
		.from(aiActionLog)
		.where(and(eq(aiActionLog.id, body.actionId), eq(aiActionLog.userId, session.user.id)))
		.limit(1);
	...
	return json({ ok: true, tool: row.tool, actionId: body.actionId });
};
