 ...
 */
async function buildPlaybackPayload({ params, locals, request, url }: RequestEvent) {
	const session = await locals.auth.getSession();
	if (!session) {
		error(401, 'Please sign in to watch content');
	}

	// A route param is `string | undefined` on a RequestHandler where it was
	// `string` on the page load this came from — SvelteKit types endpoint params
	// loosely because an endpoint can be reached by paths a page cannot. The id
	// is always present for this route, so narrow once here rather than
	// non-null-asserting at each use.
	...
	const content = await db
		...
		.from(mediaLibrary)
		.where(or(eq(mediaLibrary.id, contentId), eq(mediaLibrary.slug, contentId)))
		.then((r) => r[0]);
	...
	return {
		...
		viewerLocale,
		paywall,
		activeProfileId: locals.activeProfileId ?? url.searchParams.get('profileId') ?? undefined
	};
}

/**
 * The response contract, exported so the watch page can type `data` precisely.
 * It cannot be derived from the handler: `json()` returns a bare `Response`
 * whose `.json()` is `Promise<any>`, which would silently erase the typing the
 * previous `+page.server.ts` gave the component for free.
 */
export type PlaybackPayload = Awaited<ReturnType<typeof buildPlaybackPayload>>;

export const GET: RequestHandler = async (event) => json(await buildPlaybackPayload(event));
