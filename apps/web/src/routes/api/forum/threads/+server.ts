...
const ALLOWED_CATEGORIES = new Set([
	'getting-started', 'technical', 'content-creation', 'ministry', 'community'
]);
...
export const GET: RequestHandler = async ({ url }) => {
	...
	if (q) {
		conds.push(or(ilike(forumThreads.title, `%${q}%`), ilike(forumThreads.body, `%${q}%`))!);
	}

	const orderClause = sort === 'top'
		? [desc(forumThreads.isSticky), desc(forumThreads.likeCount), desc(forumThreads.lastReplyAt)]
		: [desc(forumThreads.isSticky), desc(forumThreads.lastReplyAt), desc(forumThreads.createdAt)];
	...
	return json({ threads: rows, page, pageSize: PAGE_SIZE, total, hasMore: page * PAGE_SIZE < total });
};
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const limit = await take(`forum-thread:${session.user.id}`, { capacity: 5, refillPerSec: 1 / 600 });
	if (!limit.allowed) return json({ error: 'Slow down — try again in a few minutes.' }, { status: 429 });

	const body = await request.json().catch(() => ({})) as { title?: string; category?: string; body?: string };
	...
	if (!title || title.length < 5 || title.length > 255) {
		return json({ error: 'Title must be 5–255 characters.' }, { status: 400 });
	}
	...
	if (text.length > 10000) return json({ error: 'Post body is too long (max 10000 characters).' }, { status: 400 });

	// AI moderation. If the agent is unreachable (returns null), default to
	// auto-approve so the forum doesn't grind to a halt — admins can review
	// hidden flags later.
	const verdict = await moderateComment(text, title).catch(() => null);
	if (verdict?.verdict === 'reject') {
		return json({ error: 'Your post was rejected by automated moderation. Please rephrase.' }, { status: 400 });
	}
	...
	}).returning({ id: forumThreads.id, status: forumThreads.status });

	return json({ success: true, id: inserted.id, status: inserted.status });
};
