...
const VALID_PROCESSORS = new Set(['paystack', 'stripe']);
...
export const PUT: RequestHandler = async ({ locals, request }) => {
	...
	if (!body.payoutProcessor || !VALID_PROCESSORS.has(body.payoutProcessor)) {
		return json({ error: 'Invalid payoutProcessor' }, { status: 400 });
	}
	...
	if (body.payoutProcessor === 'stripe' && !creator.stripePayoutsEnabled) {
		return json({
			error: 'Complete Stripe onboarding before switching to Stripe payouts'
		}, { status: 400 });
	}
	...
	if (typeof body.preferredPayoutCurrency === 'string' && /^[A-Z]{3}$/.test(body.preferredPayoutCurrency)) {
		updates.preferredPayoutCurrency = body.preferredPayoutCurrency;
	}
