...
import { reviews, mediaLibrary, abuseReports } from '$lib/db/schema/sepharstudios';
...
const ALLOWED_ACTIONS = new Set(['approve', 'hide']);
...
export const PATCH: RequestHandler = async ({ params, locals, request }) => {
	...
	const [row] = await db
		...
		.from(reviews)
		.leftJoin(mediaLibrary, eq(reviews.contentId, mediaLibrary.id))
	...
	// Auto-resolve any open abuse reports targeting this review. Resolution
	// reflects what the creator chose, so the admin queue agrees with the
	// surface state.
