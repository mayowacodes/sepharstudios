...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!tokenAddress || !isAddress(tokenAddress)) {
		return json({ ok: true, skipped: true, reason: 'STC token address not configured' });
	}
	...
	const result = { processed: 0, completed: 0, failed: 0, unmined: 0, errors: [] as string[] };
		...
		try {
			...
			if (receipt.status === 'success') {
				await db.update(transactions)
					.set({
						status: 'completed',
						metadata: sql`coalesce(${transactions.metadata}, '{}'::jsonb) || ${JSON.stringify({ reconciledAt: new Date().toISOString(), blockNumber: Number(receipt.blockNumber) })}::jsonb`
					})
				...
				await db.update(transactions)
					.set({
						status: 'failed',
						metadata: sql`coalesce(${transactions.metadata}, '{}'::jsonb) || ${JSON.stringify({ reconciledAt: new Date().toISOString(), reconciledReason: 'on-chain revert' })}::jsonb`
					})
					.where(eq(transactions.id, row.id));
				result.failed += 1;
			}
			...
			// "Transaction receipt not found" is the not-yet-mined case — try
			// again next run. Anything else gets recorded as an error.
			if (/not found|TransactionReceiptNotFound/.test(msg)) {
				result.unmined += 1;
			} else {
