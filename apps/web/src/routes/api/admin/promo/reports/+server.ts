...
import { adCampaignDaily, adCampaigns, adAdvertisers } from '$lib/db/schema/sepharstudios';
...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	const rows = await db
		...
		.leftJoin(adCampaigns, eq(adCampaigns.id, adCampaignDaily.campaignId))
		.leftJoin(adAdvertisers, eq(adAdvertisers.id, adCampaigns.advertiserId))
		.where(filters.length ? and(...filters) : undefined)
		.orderBy(desc(adCampaignDaily.day))
		.limit(1000);
	...
	const totals = rows.reduce(
		...
		{ served: 0, started: 0, completed: 0, skipped: 0, clicks: 0, watchSeconds: 0 }
	);

	return json({
		rows,
		totals,
		// Rendered as percentages by the UI. Guarded against divide-by-zero so a
		// campaign with no delivery reports 0 rather than NaN.
		rates: {
