...
async function previewFor(targetType: string, targetId: string): Promise<string | null> {
	...
	if (targetType === 'forum_thread') {
		const [r] = await db.select({ title: forumThreads.title, body: forumThreads.body }).from(forumThreads).where(eq(forumThreads.id, targetId)).limit(1);
	...
	if (targetType === 'user') {
		const [r] = await db.select({ name: user.name, email: user.email }).from(user).where(eq(user.id, targetId)).limit(1);
...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	const rows = await db
		...
		.from(abuseReports)
		.leftJoin(user, eq(abuseReports.reporterId, user.id))
		.where(where)
