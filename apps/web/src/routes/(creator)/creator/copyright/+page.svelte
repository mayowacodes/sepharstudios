...
<svelte:head>
  <title>Creator Copyright Policy · Sephar Studios</title>
...
<div class="mx-auto max-w-4xl py-6 px-4 space-y-6">
  ...
  <div class="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 flex gap-3">
    ...
    <p class="text-sm">
      If you are unsure whether you have the rights to publish something,
      <strong>don't upload it.</strong> Email <a class="underline" href="mailto:copyright@sepharstudios.com">copyright@sepharstudios.com</a>
      first — pre-submission clearance is free and avoids strikes.
    </p>
  </div>

  <ol class="space-y-6">
    {#each sections as section}
      <li class="bg-card border border-border rounded-xl p-5 space-y-2">
        <h2 class="text-lg font-semibold flex items-center gap-2">
          <FileText class="w-4 h-4 text-primary" />{section.title}
        </h2>
        <p class="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{section.body}</p>
      </li>
    {/each}
  </ol>

  <footer class="bg-card border border-border rounded-xl p-5 text-center space-y-3">
    <Mail class="w-6 h-6 text-primary mx-auto" />
    <p class="text-sm text-muted-foreground">
      Questions about anything above? Reach the copyright team directly.
    </p>
    <Button href="mailto:copyright@sepharstudios.com">Email copyright@sepharstudios.com</Button>
  </footer>
</div>
