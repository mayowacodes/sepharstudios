...
<div class="container mx-auto py-8 px-4 max-w-3xl space-y-6">
  ...
  <section class="surface-1 rounded-xl p-5 space-y-4">
    <h2 class="text-sm font-semibold text-white">Submit a new form</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        <label for="form-kind" class="block text-xs text-gray-300 mb-1">Form</label>
        <select id="form-kind" bind:value={formKind} class="w-full surface-2 rounded px-3 py-2 text-sm text-white">
      ...
      <div>
        <label for="tax-year" class="block text-xs text-gray-300 mb-1">Tax year</label>
        <input id="tax-year" type="number" bind:value={taxYear} class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
      </div>
      <div class="md:col-span-2">
        <label for="t-name" class="block text-xs text-gray-300 mb-1">Legal name *</label>
        <input id="t-name" type="text" bind:value={name} class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
      </div>
      <div class="md:col-span-2">
        <label for="t-business" class="block text-xs text-gray-300 mb-1">Business/entity name (if applicable)</label>
        <input id="t-business" type="text" bind:value={businessName} class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
      </div>
      <div class="md:col-span-2">
        <label for="t-address" class="block text-xs text-gray-300 mb-1">Address *</label>
        <input id="t-address" type="text" bind:value={address} class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
      </div>
      <div class="md:col-span-2">
        <label for="t-city" class="block text-xs text-gray-300 mb-1">City / state / ZIP</label>
        <input id="t-city" type="text" bind:value={citySZ} class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
      </div>
      <div>
        <label for="t-tin" class="block text-xs text-gray-300 mb-1">TIN / SSN / EIN *</label>
        <input id="t-tin" type="password" autocomplete="off" bind:value={tin} placeholder="XX-XXXXXXX or XXX-XX-XXXX" class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
      </div>
      <div>
        <label for="t-signed" class="block text-xs text-gray-300 mb-1">Signed as *</label>
        <input id="t-signed" type="text" bind:value={signedAs} placeholder="Type your full name" class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
      </div>
      {#if formKind === 'W-8BEN' || formKind === 'W-8BEN-E'}
        <div>
          <label for="t-country" class="block text-xs text-gray-300 mb-1">Country of residence *</label>
          <input id="t-country" type="text" bind:value={country} class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
        </div>
        <div>
          <label for="t-ftin" class="block text-xs text-gray-300 mb-1">Foreign tax ID *</label>
          <input id="t-ftin" type="password" autocomplete="off" bind:value={foreignTaxId} class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
        </div>
      {/if}
    </div>
    <div class="flex items-center justify-between">
      <p class="text-xs text-gray-500">TIN is masked at rest. Only verified admins can view your form.</p>
  ...
  <!-- Generated 1099s -->
  <section>
    <h2 class="text-sm font-semibold text-white mb-2">Annual 1099s</h2>
    {#if loading}
      <Skeleton class="h-16 rounded-xl" />
    {:else if forms1099.length === 0}
      <div class="surface-1 rounded-xl p-6 text-center text-gray-400 text-xs">
      ...
      <ul class="space-y-2">
        {#each forms1099 as f (f.id)}
          <li class="surface-1 rounded-xl p-3 flex items-center gap-3">
            ...
            <div class="flex-1">
              <div class="text-sm text-white">1099-NEC · tax year {f.taxYear}</div>
              <div class="text-xs text-gray-500">
                Total reported: <span class="text-white">{money(f.totalPaidCents)}</span>
  ...
  <!-- History -->
  <section>
    <h2 class="text-sm font-semibold text-white mb-2">Submitted forms</h2>
    {#if loading}
      <Skeleton class="h-16 rounded-xl" />
    {:else if forms.length === 0}
      <div class="surface-1 rounded-xl p-8 text-center text-gray-400 text-sm">No forms on file yet.</div>
    {:else}
      <ul class="space-y-2">
        {#each forms as f (f.id)}
          <li class="surface-1 rounded-xl p-3 flex items-center gap-3">
            ...
            <div class="flex-1">
              <div class="text-sm text-white">{f.formKind} · tax year {f.taxYear}</div>
              <div class="text-xs text-gray-500">Submitted {new Date(f.submittedAt).toLocaleDateString()}</div>
