...
<script lang="ts">
  ...
  async function handleFileUpload(event: Event, type: string) {
    ...
    // Capture the prior value so a failed upload can revert the avatar
    // instead of leaving the creator with a broken blob: preview that
    // disappears on reload.
    const previousProfileImage = profileData.personalInfo.profileImage;
    let previewObjectUrl: string | null = null;
    if (type === 'profile') {
      previewObjectUrl = URL.createObjectURL(file);
      profileData.personalInfo.profileImage = previewObjectUrl;
    }
    ...
    try {
      ...
      if (type === 'profile') {
        ...
        // Release the temporary object URL — the persisted CDN URL is now
        // in place and the blob would otherwise leak until tab close.
      ...
      if (type === 'profile') {
        profileData.personalInfo.profileImage = previousProfileImage;
        if (previewObjectUrl) URL.revokeObjectURL(previewObjectUrl);
      }
      alert(`Upload failed: ${err.message}`);
    } finally {
      // Reset the file input so the same file can be re-selected after a
      // failed upload — browsers de-dupe identical selections otherwise.
      input.value = '';
    }
  }
...
<div class="mx-auto px-4 py-6 space-y-6 max-w-5xl">
  ...
  {#if isLoading}
    ...
    <div class="flex items-center justify-center py-12">
      ...
      <p class="text-foreground ml-4">Loading profile...</p>
    </div>
  {:else}
    <!-- Tab Navigation -->
    <div class="surface-2 backdrop-blur-sm rounded-xl">
      <div class="flex flex-wrap border-b border-gray-600">
        <button 
          onclick={() => activeTab = 'personal'}
          class="px-6 py-4 font-medium transition-colors {activeTab === 'personal' ? 'text-purple-400 border-b-2 border-purple-400 surface-1' : 'text-foreground/80 hover:text-foreground'}"
        >
          👤 Personal Info
        </button>
        <button 
          onclick={() => activeTab = 'ministry'}
          class="px-6 py-4 font-medium transition-colors {activeTab === 'ministry' ? 'text-purple-400 border-b-2 border-purple-400 surface-1' : 'text-foreground/80 hover:text-foreground'}"
        >
          ⛪ Ministry Details
        </button>
        <button 
          onclick={() => activeTab = 'social'}
          class="px-6 py-4 font-medium transition-colors {activeTab === 'social' ? 'text-purple-400 border-b-2 border-purple-400 surface-1' : 'text-foreground/80 hover:text-foreground'}"
        >
          🔗 Social Links
        </button>
        <button 
          onclick={() => activeTab = 'preferences'}
          class="px-6 py-4 font-medium transition-colors {activeTab === 'preferences' ? 'text-purple-400 border-b-2 border-purple-400 surface-1' : 'text-foreground/80 hover:text-foreground'}"
        >
          ⚙️ Preferences
        </button>
      </div>
      ...
      <div class="p-6">
        {#if activeTab === 'personal'}
          ...
          <div class="space-y-6">
            <div class="rounded-lg border border-border/40 surface-1 p-4">
              <div class="text-foreground font-medium mb-2">Creator Type</div>
              <div class="flex flex-wrap gap-4 text-foreground/90">
            ...
            <div class="flex items-start space-x-6">
              <div class="shrink-0">
                <div class="w-24 h-24 rounded-full overflow-hidden bg-gray-600">
                  {#if profileData.personalInfo.profileImage}
                    ...
                    />
                  {:else}
                    <div class="w-full h-full flex items-center justify-center text-muted-foreground text-2xl">
                      👤
                    </div>
                  {/if}
                </div>
              ...
              <div>
                <div class="text-foreground font-medium mb-2">Profile Image</div>
                <input 
                  type="file" 
                  accept="image/*"
                ...
                <div class="text-muted-foreground text-sm mt-1">JPG, PNG or GIF. Max 5MB.</div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input 
                  ...
                  class="w-full px-4 py-3 surface-2 border border-gray-600 rounded-lg text-foreground placeholder-gray-400 focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                />
              </div>
            ...
            <div>
              ...
              <textarea 
                ...
                rows="4"
                class="w-full px-4 py-3 surface-2 border border-gray-600 rounded-lg text-foreground placeholder-gray-400 focus:ring-2 focus:ring-purple-600 focus:border-transparent resize-none"
              ></textarea>
              <div class="text-sm text-muted-foreground mt-1">{profileData.personalInfo.bio.length}/500 characters</div>
            </div>
          </div>
        ...
        {:else if activeTab === 'ministry'}
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label for="ministryName" class="block text-sm font-medium text-foreground mb-2">Ministry Name *</label>
                <input 
                  type="text" 
                  id="ministryName"
              ...
              <div>
                <label for="ministryWebsite" class="block text-sm font-medium text-foreground mb-2">Ministry Website</label>
                <input 
                  type="url" 
                  id="ministryWebsite"
              ...
              <div>
                <label for="denomination" class="block text-sm font-medium text-foreground mb-2">Denomination</label>
                <select 
                  id="denomination"
                  bind:value={profileData.ministryInfo.denomination}
                  class="w-full px-4 py-3 surface-2 border border-gray-600 rounded-lg text-foreground focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                >
              ...
              <div>
                <label for="yearsInMinistry" class="block text-sm font-medium text-foreground mb-2">Years in Ministry</label>
                <input 
                  type="number" 
                  id="yearsInMinistry"
            ...
            <div>
              <label for="ministryDescription" class="block text-sm font-medium text-foreground mb-2">Ministry Description</label>
              <textarea 
            ...
            <!-- Ministry Address -->
            <div>
              <label for="ministryAddress" class="block text-sm font-medium text-foreground mb-2">Ministry Address</label>
              <textarea 
                id="ministryAddress"
            ...
            <div>
              <div class="flex items-center justify-between mb-3">
                <div class="text-sm font-medium text-foreground">Verification Documents</div>
                <input
                  ...
                  {#each profileData.ministryInfo.verificationDocuments as doc, index}
                    ...
                    <div class="flex items-center justify-between surface-1 rounded-lg p-3">
                      <div class="flex items-center">
                        ...
                        <span class="text-foreground">{docName}</span>
                      </div>
                      <button 
              ...
              {:else}
                <div class="text-muted-foreground text-sm">No documents uploaded yet. Add documents to verify your ministry.</div>
              {/if}
            </div>
          </div>
        ...
        {:else if activeTab === 'social'}
          <!-- Social Links -->
          <div class="space-y-6">
            <div class="text-foreground/80 mb-6">
              Connect your social media accounts to help viewers find and follow your ministry across platforms.
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label for="youtube" class="text-sm font-medium text-foreground mb-2 flex items-center">
              ...
              <div>
                <label for="facebook" class="text-sm font-medium text-foreground mb-2 flex items-center">
              ...
              <div>
                <label for="instagram" class="text-sm font-medium text-foreground mb-2 flex items-center">
              ...
              <div>
                <label for="twitter" class="text-sm font-medium text-foreground mb-2 flex items-center">
              ...
              <div>
                <label for="website" class="text-sm font-medium text-foreground mb-2 flex items-center">
              ...
              <div>
                <label for="podcast" class="text-sm font-medium text-foreground mb-2 flex items-center">
        ...
        {:else if activeTab === 'preferences'}
          ...
          <div class="space-y-6">
            ...
            <!-- Privacy Settings -->
            <div>
              <h3 class="text-lg font-medium text-foreground mb-4">Privacy Settings</h3>
              <div class="space-y-4">
                <div class="flex items-center justify-between">
                  <div>
                    <div class="text-foreground font-medium">Public Profile</div>
                    <div class="text-muted-foreground text-sm">Make your profile visible to platform users</div>
                  </div>
                  <input 
                    type="checkbox" 
                ...
                <div class="flex items-center justify-between">
                  <div>
                    <div class="text-foreground font-medium">Show Contact Information</div>
                    <div class="text-muted-foreground text-sm">Display your email and phone on your public profile</div>
                  </div>
                  <input 
                    type="checkbox" 
            ...
            <div>
              <h3 class="text-lg font-medium text-foreground mb-4">Notifications</h3>
              <div class="space-y-4">
                <div class="flex items-center justify-between">
                  <div>
                    <div class="text-foreground font-medium">Email Notifications</div>
                    <div class="text-muted-foreground text-sm">Receive general platform notifications</div>
                  </div>
                  <input 
                    type="checkbox" 
                ...
                <div class="flex items-center justify-between">
                  <div>
                    <div class="text-foreground font-medium">Review Notifications</div>
                    <div class="text-muted-foreground text-sm">Get notified about content review status changes</div>
                  </div>
                  <input 
                    type="checkbox" 
                ...
                <div class="flex items-center justify-between">
                  <div>
                    <div class="text-foreground font-medium">Marketing Emails</div>
                    <div class="text-muted-foreground text-sm">Receive promotional content and platform updates</div>
                  </div>
                  <input 
                    type="checkbox" 
