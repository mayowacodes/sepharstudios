<script lang="ts">
  ...
  function statusBadge(s: string): string {
    ...
    if (s === 'ended') return 'bg-gray-600/40 text-gray-300';
    if (s === 'errored') return 'bg-red-600/40 text-red-100';
    return 'surface-2 text-gray-300';
  }
</script>
...
<div class="container mx-auto py-8 px-4 max-w-4xl space-y-6">
  ...
  <section class="surface-1 rounded-xl p-4 space-y-3">
    <h2 class="text-sm font-semibold text-white">New stream</h2>
    <input type="text" bind:value={newTitle} placeholder="Stream title" class="w-full surface-2 rounded px-3 py-2 text-sm text-white" />
    <textarea bind:value={newDescription} rows="2" placeholder="Description (optional)" class="w-full surface-2 rounded px-3 py-2 text-sm text-white resize-none"></textarea>
  ...
  <!-- List -->
  <section>
    <h2 class="text-sm font-semibold text-white mb-3">Your streams</h2>
    {#if loading}
      <div class="space-y-2">
        {#each Array(2) as _ (_)}<Skeleton class="h-32 rounded-xl" />{/each}
      </div>
    {:else if streams.length === 0}
      <div class="surface-1 rounded-xl p-10 text-center text-gray-400">No streams yet. Create one above to get started.</div>
    {:else}
      <ul class="space-y-3">
          ...
          <li class="surface-1 rounded-xl p-4 space-y-3">
            <div class="flex items-start justify-between gap-2">
              ...
              <div class="flex gap-1">
                <button type="button" onclick={() => rotateKey(s)} title="Rotate stream key" class="text-yellow-300 hover:text-yellow-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              <div class="surface-2 rounded p-2 flex items-center gap-2">
                <div class="flex-1 min-w-0">
                  <div class="text-gray-400 text-[10px] uppercase">RTMP URL</div>
                  <div class="text-white font-mono truncate">{s.rtmpIngestUrl ?? '—'}</div>
                </div>
                {#if s.rtmpIngestUrl}
              ...
              <div class="surface-2 rounded p-2 flex items-center gap-2">
                <div class="flex-1 min-w-0">
                  <div class="text-gray-400 text-[10px] uppercase">Stream key (secret)</div>
                  <div class="text-white font-mono truncate">{s.streamKey}</div>
                </div>
