...
<div class="mx-auto px-4 py-6 space-y-6 max-w-7xl">
  ...
  <div class="flex flex-wrap gap-2">
    ...
    <StatChip label="family safety" value={policies.filter(p => p.category === 'family_safety').length} tone="red" />
    <StatChip label="content" value={policies.filter(p => p.category === 'content').length} tone="green" />
    <StatChip label="technical" value={policies.filter(p => p.category === 'technical').length} tone="blue" />
  </div>
  
  <!-- Filter -->
  <div class="surface-2 rounded-xl p-6">
    <div class="flex items-center space-x-4">
      <label for="filterByCategory" class="text-sm font-medium text-foreground">Filter by Category:</label>
      <select 
        id="filterByCategory"
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    {#each filteredPolicies as policy}
      <div class="surface-2 rounded-xl p-6 space-y-4">
        <div class="flex justify-between items-start">
          <div>
            <div class="flex items-center space-x-3 mb-2">
              <h3 class="text-xl font-bold text-foreground">{policy.title}</h3>
              <span class="px-3 py-1 text-xs rounded-full {getCategoryColor(policy.category)}">
                {policy.category.replace('_', ' ')}
              </span>
            </div>
            <p class="text-foreground/80 text-sm">{policy.description}</p>
          </div>
          ...
          <div class="flex items-center space-x-2">
            ...
            <button 
              ...
              class="px-3 py-1 text-xs rounded-full transition-colors {policy.isActive ? 'bg-green-600 text-foreground' : 'bg-gray-600 text-white/80'}"
            >
        ...
        <!-- Requirements -->
        <div>
          <h4 class="font-medium text-foreground mb-2">Requirements</h4>
          <ul class="text-sm text-green-300 space-y-1">
            ...
            {#if policy.requirements.length > 3}
              <li class="text-muted-foreground">... and {policy.requirements.length - 3} more</li>
            {/if}
          </ul>
        </div>
        ...
        <!-- Common Violations -->
        <div>
          <h4 class="font-medium text-foreground mb-2">Common Violations</h4>
          <ul class="text-sm text-red-300 space-y-1">
            {#each policy.violations.slice(0, 2) as violation}
              <li>• {violation}</li>
            {/each}
            {#if policy.violations.length > 2}
              <li class="text-muted-foreground">... and {policy.violations.length - 2} more</li>
            {/if}
          </ul>
        </div>
    ...
    <div class="bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      <div class="p-6">
        <div class="flex justify-between items-center mb-6">
          ...
          <button onclick={cancelEdit} class="text-muted-foreground hover:text-foreground" aria-label="Close policy modal">
        ...
        <div class="space-y-4">
          <!-- Title -->
          <div>
            <label for="policyTitle" class="block text-sm font-medium text-foreground mb-2">Policy Title</label>
            <input 
              id="policyTitle"
              type="text" 
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label for="policyCategory" class="block text-sm font-medium text-foreground mb-2">Category</label>
              <select 
                id="policyCategory"
            ...
            <div>
              <label for="policySeverity" class="block text-sm font-medium text-foreground mb-2">Severity</label>
              <select 
                id="policySeverity"
          ...
          <!-- Description -->
          <div>
            <label for="policyDescription" class="block text-sm font-medium text-foreground mb-2">Description</label>
            <textarea 
