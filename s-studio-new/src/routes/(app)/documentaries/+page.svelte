<script lang="ts">
    ...
    async function fetchDocumentaries() {
      ...
      // const response = await fetch(`/api/documentaries?category=${$selectedCategory}&topic=${$selectedTopic}`);
      // const apiDocs = await response.json();
      // allDocumentaries = [...documentaries, ...apiDocs];
    }
  
    $: selectedCategory, selectedTopic, fetchDocumentaries();
  ...
  <main class="container mx-auto px-4 py-8">
    ...
    {:else}
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {#each $filteredDocumentaries as documentary}
          <DocumentaryCard {documentary} />
        {/each}
      </div>
    {/if}
  </main>